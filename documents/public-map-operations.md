...
- Verified public-pin aliases live in `documents/public-pin-aliases.md`; use them only for individually reviewed marine facilities with live provider evidence, nearby negative checks, unit tests, and the normal public-pin apply gate. The first alias is `burnham yacht harbour`. Do not add global `yacht harbour` or `yacht harbor` matching from this evidence.
