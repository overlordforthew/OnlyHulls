...
- Current approved cleanup scope is narrow: recurring `Nanny Cay Boatyard` and `Marina Bas-Du-Fort` source artifacts may canonicalize before geocoding; one-off production text fixes are backed by `boat_location_text_cleanup_backup_20260421014841` and `boat_location_text_cleanup_backup_20260421020349`. Green Cay, St. Thomas Yacht Club, Puerto Bahia, Clarke's Court, Riva Di Traiano, and Villanova remain review items, not public-pin normalization rules.
