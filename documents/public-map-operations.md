...
- Latest backlog planning checkpoint: `reports/location-backlog/2026-04-21-round20.{json,md}` still keeps the public map disabled after the Linton Bay Marina apply and Lagoon repair. It found 12,666 active visible rows, 263 public pins, 371 public-pin-likely rows, 0 country-hint mismatches, 2,993 pending-ready rows, 373 review rows, and 510 held-back coordinates. The recommended next move remains `cleanup_first`; remaining work is raw source cleanup, search-coverage batches, and smaller vetted gazetteer POI clusters.
...
- Latest verified search-coverage checkpoint: `boat_geocode_backup_20260421123258` retried only reviewed rows whose normalized query changed after round 18 cleanup; it wrote 5 city search-only coordinates, held 1 low-confidence region result in review with null coordinates, found 0 failed rows and 0 geography mismatches, and created 0 public pins. The follow-up changed-review retry selected 0 rows, readiness remains `NO_GO_KEEP_PUBLIC_MAP_DISABLED`, public pins stayed at 248, raw coordinates increased to 766, held-back coordinates increased to 518, and `/api/boats/map` still returns 404 while disabled.
