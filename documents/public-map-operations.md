...
- The exact `City, Country` confidence-3 path is a closed search-coverage exception, not a public-pin shortcut. It requires a two-part query, matching country hint/provider country code, city precision, result text that names the queried city and country, and no street, POI, marina, or marine components. Do not extend it to marina/street/exact public-pin work.
...
- Verified public-pin aliases live in `documents/public-pin-aliases.md`; use them only for individually reviewed marine facilities with live provider evidence, nearby negative checks, unit tests, and the normal public-pin apply gate. Current aliases are `burnham yacht harbour`, `conwy marina`, `chichester marina`, `palm cay marina`, `medway yacht club`, `lagoon marina`, `marina frapa`, `marina baotic`, `linton bay marina`, and `mdl chatham maritime marina boatyard`. Do not add global `yacht harbour`, `yacht harbor`, or broad `marina` matching from this evidence.
...
- Use `--boat-id=<uuid>` or comma-separated `--boat-ids=<uuid>,<uuid>` for surgical production repairs when a consensus gate requires exact row targeting. Round 20 added this after the rejected Cole Bay metadata experiment so a repair could not widen beyond the two affected Lagoon rows.
- Use `--retry-changed-geocoded` only with `--verified-public-pin-aliases` for existing non-public city/region/country/unknown coordinates whose recomputed query changed. The lane is capped to `--limit=50`, excludes existing public-grade pins, and requires the verified alias in both source text and recomputed query.
...
- Current approved cleanup scope is narrow: recurring `Nanny Cay`/`Nanny Cay Boatyard`, `Marina Bas-Du-Fort`, `Hodge's Creek Marina Hotel`, `Camper & Nicholsons Port Louis Marina`, `Zea Marina`, `La Paz, Costa Baja Marina, Americas`, `La Cruz Marina Near Puerto Vallarta`, dirty `Shelter Bay Marina` variants, `Linton Bay Marina Garrote Coln`, `Verkoophaven Delta Marina` and `Schepenkring Delta Marina` variants, `Marina Vaiare`, `Chiapas Marina`, `Puerto Escondido Loreto Marina, BCS`, `Bluffers Park Yacht Club`, `Foxs Marina, Ipswich`, `Penarth Marina Cardiff`, `Greystones Harbour Marina`, `Northern Ireland, Carrickfergus Marina`, `Clarke's Court Boatyard & Marina`, `British Virgin Islands, Hodge's Creek Marina, Caribbean`, `Dubrovnik, Komolac, ACI Marina Dubrovnik`, `Šibenik, Marina Zaton, Mediterranean`, `Kos, Kos Marina, Mediterranean`, `Marsh Harbour, Conch Inn Marina, Bahamas`, `Port Tino Rossi`, `Port Pin Rolland`, `Marmaris Yacht Marine`, `Le Marin Martiniqe`, `Marina De L'Anse Marcel`, exact `Chatham Marina, Kent` / `Chatham Marina, Chatham Kent` rows, Dutch `Aan Verkoopsteiger In` / `Aaan Verkoopsteiger In` prefixes, Saint Martin/Sint Maarten suffix variants, misspelled place tokens (`Bahmas`, `Martiniqe`, `El Hiero`, `Santantioco`, scoped `Grenade Caribbean`), anchored Canadian Lake Huron/Osoyoos rows, directional fragments such as `North East Of`, and exact `Trogir, Yachtclub Seget (Marina Baotić)` rows may canonicalize before geocoding; one-off production text fixes are backed by `boat_location_text_cleanup_backup_20260421014841`, `boat_location_text_cleanup_backup_20260421020349`, and `boat_location_text_cleanup_backup_20260421022049`. Green Cay, St. Thomas Yacht Club, Puerto Bahia, broad Clarke's Court rows, Riva Di Traiano, Villanova, BVI Yacht Charter Docks, Marina Di Ragusa, Sune Carlsson Boatyard, Red Frog Marina, Pankor/Pangkor Marina, Ocean Marina Pattaya, Wiarton/Wiatron Marina, Marina Del Ray Orillia, Marina Riviera Nayarit, Marina Guaymas, Rio Dulce Marina, Swanwick Marina, D-Marin Lefkas, Dover Marina, plain Chatham Marina without Kent/country context, Port Solent Marina, Shotley Marina, Carlyle West Access Marina, Shores Of Leech Lake Marina, Tollesbury Marina, Deko Marina, Carrickfergus Harbour Marina, Roses/Empuriabrava Marina, Northshore Shipyard Chichester, and Pembroke Dock remain review items, not public-pin normalization rules. Verified aliases such as Conwy Marina, Chichester Marina, Palm Cay Marina, Medway Yacht Club, Lagoon Marina, Marina Frapa, Marina Baotić, Linton Bay Marina, and MDL Chatham Maritime Marina Boatyard must stay in the alias lane rather than this cleanup-normalization lane. Do not add a broad `Cole Bay` or `Lagoon Marina Cole Bay` market alias until a facility-specific geocode path is proven; the round 20 `Sint Maarten`-suffixed query returned city precision.
- Latest verified public-pin alias checkpoint: `boat_geocode_backup_20260421213442` wrote 2 Round 25 marina-grade pins (1 Alcaidesa Marina (Spain, row tagged Gibraltar) and 1 D-Marin Didim Marina (Turkey)) with `PUBLIC_MAP_ENABLED=false`; 0 held back, 0 failed, 0 geography mismatches, 0 warnings, 2/2 backup-scoped audit eligible. Public pins increased to 275. Round 25 added two anchored canonicalization rules for the dirty production source texts and exercised the Round 23 `providerCountryCodes` widening for Alcaidesa (second multi-country alias consumer after Green Cay). Round 25 also deferred Empuriabrava Marina, Ensenada Cruiseport Marina, Fort Pierce City Marina, Liberty Harbor Marina, Felixstowe Ferry Boatyard, Cathedral Bluffs Yacht Club, and Etobicoke Yacht Club (bare query returns a different yacht club — unsafe).
- Prior verified public-pin alias checkpoint: `boat_geocode_backup_20260421194136` wrote 2 Toronto Island Marina (Canada) marina-grade pins with `PUBLIC_MAP_ENABLED=false`; 0 rows were held back, 0 failed, 0 geography mismatches were found, the verified-alias lane selected 0 rows afterward, and the backup-scoped audit returned 2/2 eligible pins. Public pins increased to 273. Round 24 used an anchored `^\s*Toronto\s+Island\s+Marina\s*$` canonicalization rule so the two pending bare source-text rows resolve to the facility-grade provider query; qualified variants pass through unchanged. Round 24 also deferred Red Frog Marina, Pembroke Dock, Tollesbury Marina, and Sune Carlsson Boatyard for lack of facility-grade OpenCage evidence.
- Prior verified public-pin alias checkpoint: `boat_geocode_backup_20260421174140` wrote 4 Green Cay Marina (USVI) marina-grade pins with `PUBLIC_MAP_ENABLED=false`; 0 rows were held back, 0 failed, 0 geography mismatches were found, the verified-alias lane selected 0 rows afterward, and the backup-scoped audit returned 4/4 eligible pins. Public pins increased to 271. Round 23 also introduced a narrow provider-side country-filter widening (`GeocodeQuery.providerCountryCodes`) that only activates when a query text contains a verified alias whose `countryCodes` has more than one entry (currently just `lagoon marina` (nl+sx) and `green cay marina` (us+vi)); non-alias queries, single-country aliases, and the alias anchor check are unaffected. This was required because OpenCage tags USVI facilities as `country_code=us` so a `vi`-only filter de-ranks the marina below Christiansted city.
- Prior verified public-pin alias checkpoint: `boat_geocode_backup_20260421152318` wrote 4 MDL Chatham Maritime Marina Boatyard marina-grade pins with `PUBLIC_MAP_ENABLED=false`; 0 rows were held back, 0 failed, 0 geography mismatches were found, the verified-alias lane selected 0 rows afterward, and the backup-scoped audit returned 4/4 eligible pins. Public pins increased to 267 and active-visible public coverage reached 2.11%.
- Latest backlog planning checkpoint: `reports/location-backlog/2026-04-21-round23.{json,md}` still keeps the public map disabled after the Green Cay Marina alias apply. The recommended next move remains `gazetteer_first`; remaining deferred clusters Marina Anse Marcel (Lonvilliers Harbour `_type=water` only; not marina-grade), Marina Del Rey (Los Angeles County neighbourhood only), Dover Marina (OpenCage returns `Dover Pier, New Marina Curve Road` road-text), and plain Chatham Marina without country still need live provider proof or enrichment before any public-pin apply.
