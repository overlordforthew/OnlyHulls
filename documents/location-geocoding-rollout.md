...
- `boat_geocode_backup_20260421225024`, `..._20260421225038`: two Round 32 `--include-review --limit=500 --apply` batches with `PUBLIC_MAP_ENABLED=false`. Aggregate: 500 rows selected, 24 geocoded (18+6), 476 held in review, 0 failed. Round 32 extends `allowsExactCityCountryConfidenceFloor` to accept `confidence=2` matches when the primary city component (`city`/`town`/`village`/`municipality`/`hamlet`) value EXACTLY equals the queried city part. The conf=3 path keeps its looser check (city name present anywhere in result text); conf=2 requires strict component equality to prevent ambiguous-name traps (e.g. `Bursa, Turkey` returning `Mudanya, Bursa, Turkey` with `city=Mudanya` would fail the new guard even though the broader text contains "Bursa"). Also updated a pre-existing test case from conf=2 to conf=1 to keep "unsafe confidence" coverage at a still-rejected threshold. 5 stale cache entries (Bodrum/Fethiye/Marmaris/Murcia/Palma De Mallorca) snapshotted to `location_geocode_cache_r32_conf2_backup` and deleted; re-fetches went through the extended classifier. Readiness deltas: publicPinCount unchanged at 280, rawCoordinateCount 2007→2031, **review_failed_rate 33.76%→32.97%**. Small gain per round but real; going forward any NEW rows hitting the 5 targeted clusters (or similar city+component matches) will geocode instead of going to review.
- `boat_geocode_backup_20260421231900..._20260421232339` (series of Round 33 batches): ran 6 unfocused `--include-review --limit=500` batches that hit the same deterministic 250-row slice each time (conf=1 stuck rows — Liguria, French Riviera, Mallorca — that the current classifier correctly rejects). **Solution: targeted --boat-ids** — after identifying 119 review boats whose stored query had an already-geocoded cache entry, fed them explicitly via `--boat-ids=<119 comma-separated UUIDs>`. Final backup: `boat_geocode_backup_20260421232339`. 119 boats processed, 119 geocoded, 0 review, 0 provider fetches (all cache hits). **review_failed_rate 32.97%→28.98%** — biggest single-round drop of the series. Lesson: the script's `--include-review` selection is deterministic and caps at 250 rows per batch; if the first 250 hit legitimately-unfixable rows, subsequent batches don't advance. Surgical `--boat-ids` lists bypass that and drain newly-classifier-eligible boats directly.
...
- `boat_geocode_backup_20260421233700`: Round 34 extended the exact City,Country exception to accept 3-part `City, Region, Country` queries with the same guards as 2-part (country code match, city-part not country/broad/marine/street, region-part a real token not another country or broad). 23 stale 3-part cache entries (Alicante, Cartagena Murcia, Palma variants, Hyères, Olbia Sardinia, etc.) snapshotted to `location_geocode_cache_r34_3part_backup` then deleted; targeted `--boat-ids=<89 UUIDs>` drain re-fetched via extended classifier. Result: 89 selected, 63 geocoded, 26 still held (Palma variants where primary component `Palma de Mallorca` doesn't exactly equal queried `Palma`, Road Town BVI where cc=gb doesn't match expected `vg`, etc.). **review_failed_rate 28.98%→26.9%.** Three new regression tests: positive (Alicante), same-country wrong-city (Valencia), and middle-is-country malformed (rejected).
- `boat_geocode_backup_20260421234746`: Round 35 added overseas-territory ↔ sovereign country-code equivalence to `allowsExactCityCountryConfidenceFloor`. Territory-sovereign pairs: mq↔fr (Martinique), gp↔fr (Guadeloupe), pf↔fr (French Polynesia), re↔fr (Réunion), nc↔fr (New Caledonia), bl↔fr (St Barthélemy), mf↔fr (St Martin), pm↔fr (St Pierre and Miquelon), yt↔fr (Mayotte), vg↔gb (BVI), vi↔us (USVI), sx↔nl (Sint Maarten), cw↔nl (Curaçao), aw↔nl (Aruba), pr↔us (Puerto Rico), gu↔us (Guam). Two new regression tests: positive (Le Marin, Martinique with provider cc=fr accepted) and negative (Marin, Pontevedra, Spain with provider cc=es rejected — equivalence is not transitive to other countries). 3 stale cache entries (Le Marin Martinique, Road Town British Virgin Islands, Road Town Tortola BVI) snapshotted to `location_geocode_cache_r35_territory_backup`, deleted, and re-fetched via extended classifier. Targeted `--boat-ids=<67 UUIDs>` drain: 67 selected, 67 geocoded, 0 review, 0 failed, 0 warnings. **review_failed_rate 26.9%→24.69%** — second-biggest single-round drop. publicPinCount unchanged at 280 (all city-precision search coverage).
