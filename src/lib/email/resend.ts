...
function getFrom() {
  return process.env.RESEND_FROM_EMAIL || "DateMyBoat <hello@namibarden.com>";
}
  ...
  return getResend().emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #0369a1;">New Buyer Match on DateMyBoat</h2>
        ...
        <p style="color: #94a3b8; font-size: 12px;">DateMyBoat — AI-Powered Boat Matchmaking</p>
      </div>
    `,
  });
}
...
export async function sendIntroductionEmail(params: {
  ...
  return getResend().emails.send({
    from: getFrom(),
    to: emails,
    subject: `DateMyBoat Introduction: ${params.boatTitle}`,
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #0369a1;">You've Been Connected!</h2>
        <p><strong>${params.buyerName || "A buyer"}</strong> and <strong>${params.sellerName || "a seller"}</strong> have been matched on DateMyBoat.</p>
        <div style="background: #f1f5f9; padding: 16px; border-radius: 8px; margin: 16px 0;">
        ...
        <p style="color: #94a3b8; font-size: 12px;">DateMyBoat — Be the matchmaker, not the broker.</p>
      </div>
    `,
  });
}
