...
function repairCompoundBrandMakeModel(input: {
  ...
  if (/^pacific(?:\s+boats)?$/i.test(make) && modelStartsWith(/^(?:boats(?:\s+inc\.?)?|inc\.?)\b[\s-]*/i)) {
    make = "Pacific Boats";
    model = model.replace(/^(?:boats(?:\s+inc\.?)?|inc\.?)\b[\s-]*/i, "").trim();
  }

  if (/^bluewater(?:\s+boats)?$/i.test(make) && modelStartsWith(/^boats\b[\s-]*/i)) {
  ...
  if (/^holby(?:\s+marine)?$/i.test(make) && modelStartsWith(/^marine\b[\s-]*/i)) {
    make = "Holby Marine";
    model = model.replace(/^marine\b[\s-]*/i, "").trim();
  }

  if (/^fairways(?:\s+marine)?$/i.test(make) && modelStartsWith(/^marine\b[\s-]*/i)) {
    make = "Fairways Marine";
    model = model.replace(/^marine\b[\s-]*/i, "").trim();
  }

  if (/^alliaura(?:\s+marine)?$/i.test(make) && modelStartsWith(/^marine\b[\s-]*/i)) {
    make = "Alliaura Marine";
    model = model.replace(/^marine\b[\s-]*/i, "").trim();
  }

  if (/^jeantot(?:\s+marine)?$/i.test(make) && modelStartsWith(/^marine\b[\s-]*/i)) {
    make = "Jeantot Marine";
    model = model.replace(/^marine\b[\s-]*/i, "").trim();
  }

  if (/^corsair(?:\s+marine)?$/i.test(make) && modelStartsWith(/^marine\b[\s-]*/i)) {
    make = "Corsair Marine";
    model = model.replace(/^marine\b[\s-]*/i, "").trim();
  }
  ...
  if (
    /^(?:gilbert|gibert)(?:\s+marine)?$/i.test(make) &&
    modelStartsWith(/^marine(?:\s+france)?\s+gib(?:['\s-]*)sea(?:(?=\d)|\b)/i)
  ) {
    make = "Gibsea";
    model = model
      .replace(/^marine(?:\s+france)?\s+gib(?:['\s-]*)sea(?:(?=\d)|\b)[\s-]*/i, "")
      .trim();
  }
  ...
  if (/^ice(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
    make = "Ice Yachts";
    model = model.replace(/^yachts\b[\s-]*/i, "").trim();
  }

  if (/^pegasus(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
    make = "Pegasus Yachts";
    model = model.replace(/^yachts\b[\s-]*/i, "").trim();
  }

  if (/^sly(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
    make = "Sly Yachts";
    model = model.replace(/^yachts\b[\s-]*/i, "").trim();
  }

  if (/^vaan(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
    make = "Vaan Yachts";
    model = model.replace(/^yachts\b[\s-]*/i, "").trim();
  }
  ...
  if (/^neo(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
    make = "Neo Yachts";
    model = model.replace(/^yachts\b[\s-]*/i, "").trim();
  }

  if (/^swallow(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
    make = "Swallow Yachts";
    model = model.replace(/^yachts\b[\s-]*/i, "").trim();
  }

  if (/^ranger(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
    make = "Ranger Yachts";
    model = model.replace(/^yachts\b[\s-]*/i, "").trim();
  }

  if (/^ksenia(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
    make = "Ksenia Yachts";
    model = model.replace(/^yachts\b[\s-]*/i, "").trim();
  }

  if (/^alloy(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
    make = "Alloy Yachts";
    model = model.replace(/^yachts\b[\s-]*/i, "").trim();
  }

  if (/^adventure(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
    make = "Adventure Yachts";
    model = model.replace(/^yachts\b[\s-]*/i, "").trim();
  }

  if (/^rossiter(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
  ...
  if (/^nauta(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
    make = "Nauta Yachts";
    model = model.replace(/^yachts\b[\s-]*/i, "").trim();
  }

  if (/^mastori(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
    make = "Mastori Yachts";
    model = model.replace(/^yachts\b[\s-]*/i, "").trim();
  }

  if (/^morris(?:\s+yachts)?$/i.test(make) && modelStartsWith(/^yachts\b[\s-]*/i)) {
    make = "Morris Yachts";
    model = model.replace(/^yachts\b[\s-]*/i, "").trim();
  }
  ...
  if (
    sourceSite === "theyachtmarket" &&
    /^m\s+a(?:\s+t(?:\s+yachts)?)?$/i.test(make) &&
    modelStartsWith(/^t\s+yachts\b[\s-]*1220\b/i) &&
    /(?:^|-)m-a-t-yachts-1220(?:-|$)/.test(slug)
  ) {
    make = "M A T Yachts";
    model = model.replace(/^t\s+yachts\b[\s-]*/i, "").trim();
  }
  ...
  if (
    sourceSite === "theyachtmarket" &&
    /^more(?:\s+boats)?$/i.test(make) &&
    modelStartsWith(/^boats\b[\s-]*55\b/i) &&
    /(?:^|-)more-boats-55(?:-|$)/.test(slug)
  ) {
    make = "More Boats";
    model = model.replace(/^boats\b[\s-]*/i, "").trim();
  }
  ...
  if (
    ...
    modelStartsWith(/^marine\b[\s-]*323\b/i) &&
    /(?:^|-)morgan-marine-323(?:-|$)/.test(slug)
  ) {
    make = "Morgan Marine";
    model = "323";
  }
  ...
  if (
    ...
    /(?:^|-)tradition-marine-tm-42(?:-|$)/.test(slug)
  ) {
  ...
  if (
    sourceSite === "theyachtmarket" &&
    /^nortech(?:\s+marine)?$/i.test(make) &&
    modelStartsWith(/^marine\b[\s-]*hood\b[\s-]*50\b/i) &&
    /(?:^|-)nortech-marine-hood-50(?:-|$)/.test(slug)
  ) {
    make = "Nortech Marine";
    model = model.replace(/^marine\b[\s-]*/i, "").trim();
  }
  ...
  if (
    sourceSite === "theyachtmarket" &&
    /^king(?:\s+marine)?$/i.test(make) &&
    modelStartsWith(/^marine\b[\s-]*nmyd\b[\s-]*irc\b[\s-]*one\b[\s-]*off\b[\s-]*54\b/i) &&
    /(?:^|-)king-marine-nmyd-irc-one-off-54(?:-|$)/.test(slug)
  ) {
    make = "King Marine";
    model = model.replace(/^marine\b[\s-]*/i, "").trim();
  }
  ...
  if (
    sourceSite === "theyachtmarket" &&
    /^squalt(?:\s+marine)?$/i.test(make) &&
    modelStartsWith(/^marine\b[\s-]*ck\b[\s-]*64\b/i) &&
    /(?:^|-)squalt-marine-ck-64(?:-|$)/.test(slug)
  ) {
    make = "Squalt Marine";
    model = model.replace(/^marine\b[\s-]*/i, "").trim();
  }
  ...
  if (
    sourceSite === "theyachtmarket" &&
    /^green(?:\s+marine)?$/i.test(make) &&
    modelStartsWith(/^marine\b[\s-]*volvo\b[\s-]*65\b/i) &&
    /(?:^|-)green-marine-volvo-65(?:-|$)/.test(slug)
  ) {
    make = "Green Marine";
    model = model.replace(/^marine\b[\s-]*/i, "").trim();
  }
  ...
  if (
    sourceSite === "theyachtmarket" &&
    /^blakes(?:\s+marine)?$/i.test(make) &&
    modelStartsWith(/^marine\b[\s-]*cruising\s+folkboat\b/i) &&
    /(?:^|-)blakes-marine-cruising-folkboat(?:-|$)/.test(slug)
  ) {
    make = "Blakes Marine";
    model = model.replace(/^marine\b[\s-]*/i, "").trim();
  }
  ...
  if (
    sourceSite === "theyachtmarket" &&
    /^shark(?:\s+marine)?$/i.test(make) &&
    modelStartsWith(/^marine\b[\s-]*50\b/i) &&
    /(?:^|-)shark-marine-50(?:-|$)/.test(slug)
  ) {
    make = "Shark Marine";
    model = model.replace(/^marine\b[\s-]*/i, "").trim();
  }
  ...
  if (
    sourceSite === "sailboatlistings" &&
    /^nordest(?:\s+marine)?$/i.test(make) &&
    /^marine\s+crowther\s+63$/i.test(model) &&
    /(?:^|-)nordest-marine-crowther-63(?:-|$)/.test(slug)
  ) {
    make = "Nordest Marine";
    model = "Crowther 63";
  }
  ...
  if (
    sourceSite === "theyachtmarket" &&
    /^alesia(?:\s+marine)?$/i.test(make) &&
    modelStartsWith(/^marine\b[\s-]*goelette\b[\s-]*50\b/i) &&
    /(?:^|-)alesia-marine-goelette-50(?:-|$)/.test(slug)
  ) {
    make = "Alesia Marine";
    model = model.replace(/^marine\b[\s-]*/i, "").trim();
  }
  ...
  if (
    sourceSite === "theyachtmarket" &&
    /^technologie(?:\s+marine)?$/i.test(make) &&
    modelStartsWith(/^marine\b[\s-]*entincelle\b[\s-]*60\b/i) &&
    /(?:^|-)technologie-marine-entincelle-60(?:-|$)/.test(slug)
  ) {
  ...
  if (
    sourceSite === "theyachtmarket" &&
    /^asia(?:\s+catamarans)?$/i.test(make) &&
    modelStartsWith(/^catamarans\b[\s-]*stealth\b[\s-]*x\b[\s-]*47\b/i) &&
    /(?:^|-)asia-catamarans-stealth-x-47(?:-|$)/.test(slug)
  ) {
    make = "Asia Catamarans";
    model = model.replace(/^catamarans\b[\s-]*/i, "").trim();
  }

  if (
    sourceSite === "theyachtmarket" &&
    /^antares(?:\s+catamarans)?$/i.test(make) &&
    modelStartsWith(/^catamarans\b[\s-]*44\b[\s-]*gs\b/i) &&
    /(?:^|-)antares-catamarans-44-gs(?:-|$)/.test(slug)
  ) {
  ...
  if (
    sourceSite === "theyachtmarket" &&
    /^dolphin(?:\s+catamarans)?$/i.test(make) &&
    modelStartsWith(/^catamarans\b[\s-]*dolphin\b[\s-]*460\b/i) &&
    /(?:^|-)dolphin-catamarans-dolphin-460(?:-|$)/.test(slug)
  ) {
...
function stripSourceSpecificNoise(sourceSite: string | null | undefined, make: string, model: string) {
  ...
  if (/^cheoy lee$/i.test(make)) {
    cleaned = cleaned.replace(/^lee\b[\s-]*/i, "");
  }
  ...
  if (/^pegasus yachts$/i.test(make)) {
    cleaned = cleaned.replace(/^pegasus\b[\s-]*/i, "");
  }
  ...
  if (/^neo yachts$/i.test(make)) {
    cleaned = cleaned.replace(/^neo\b[\s-]*/i, "");
  }

  if (/^gibsea$/i.test(make)) {
    cleaned = cleaned.replace(/^gib(?:['\s-]*)sea(?:(?=\d)|\b)[\s-]*/i, "");
  }
